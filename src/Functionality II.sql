...
DROP TRIGGER IF EXISTS change_duration_after_arrival_update;
SHOW TRIGGERS;	
...
update tickets, (select price from  tickets where ticket_id = 12) as p set tickets.price = p.price * 1.25 where ticket_id = 12;
...
/*Otra instancia*/
SET @flight_id_1 = 36 /*id = 1*/

START TRANSACTION;
SELECT price FROM tickets where ticket_id = @flight_id_1; 
COMMIT;
...
START TRANSACTION;
UPDATE tickets SET price = price-20 where ticket_id = @flight_id_1;
DO SLEEP(7); /*espera 7 seconds*/
ROLLBACK; 
/*
